...
-- -----------------------------------------------------
-- Table `db_equatorial`.`instalacoes_ativos`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db_equatorial`.`instalacoes_ativos` (
  `id_instalacoes_ativos` INT NOT NULL AUTO_INCREMENT,
  `id_instalacoes` INT NOT NULL,
  `nm_ativo` VARCHAR(60) NULL,
  `sigla_ativo` VARCHAR(45) NULL,
  `codigo_equatorial` VARCHAR(21) NULL,
  PRIMARY KEY (`id_instalacoes_ativos`),
  INDEX `fk_instalacoes_ativos_instalacoes1_idx` (`id_instalacoes` ASC),
  CONSTRAINT `fk_instalacoes_ativos_instalacoes1`
  FOREIGN KEY (`id_instalacoes`)
  REFERENCES `db_equatorial`.`instalacoes` (`id_instalacoes`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
  ENGINE = InnoDB;
